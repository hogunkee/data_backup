...
db = client['hackathon']
d1 = '2018-06-01T:00:00:00Z'
d2 = '2018-07-09T:00:00:00Z'

## load news data ##
def load_text(is_trainset):
    total_text = []
    result = []
    for idx in range(10):
        company = STOCK_LIST[idx].lower()
        if is_trainset:
            rows = db.news.find({'symbol': company, 'date': {'$lt':d1}}, {'date': 1, 'title': 1, 'text': 1}).sort('date')
        else:
            rows = db.news.find({'symbol': company, 'date': {'$gt':d1, '$lt':d2}}, {'date': 1, 'title': 1, 'text': 1}).sort('date')
        ...
        result.append([dates, titles, texts])

        total_text += texts
        total_text += titles

    return result, total_text
...
def stock_data():
    result = []
    for idx in range(10):
        company = STOCK_LIST[idx]
        stock_dir = '../data/' + company + '.csv'
        date, stock = load_stock(stock_dir)
        result.append(get_stock_slope(date, stock))
    return result
